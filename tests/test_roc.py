...
from score_analysis import Scores, roc
...
def test_roc(fnr, fpr, thresholds):
    ...
    roc_curve = roc.roc(scores, fnr=fnr, fpr=fpr, thresholds=thresholds)
...
def test_roc_nb_points():
    scores = Scores(pos=[1, 2, 3], neg=[-1, -2, -3])
    roc_curve = roc.roc(scores, nb_points=10)
    assert len(roc_curve.fnr) == len(roc_curve.fpr) == 10

    roc_curve = roc.roc(scores, fnr=[], fpr=[], thresholds=[], nb_points=12)
...
def test_roc_with_ci(fpr, fnr):
    ...
    roc_curve = roc.roc_with_ci(scores, fpr=fpr, fnr=fnr, alpha=0.1)
...
def test_roc_with_ci_2():
    ...
    roc.roc_with_ci(scores, fpr=np.logspace(-2, -1))
